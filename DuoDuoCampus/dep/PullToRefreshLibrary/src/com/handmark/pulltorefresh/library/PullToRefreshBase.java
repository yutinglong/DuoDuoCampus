...
import com.handmark.pulltorefresh.library.internal.MLSTweenAnimLoadingLayout;
...
//					return new RotateLoadingLayout(context, mode, scrollDirection, attrs);
					return new MLSTweenAnimLoadingLayout(context, mode, scrollDirection, attrs);
				case FLIP:
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (mStartTime == -1) {
				...
				if (mCurrentY != 0 && !isRefreshing()) {
					float scale = Math.abs(mCurrentY) / (float) itemDimension;
					switch (mCurrentMode) {
						case PULL_FROM_END:
							mFooterLayout.onPull(scale);
							break;
